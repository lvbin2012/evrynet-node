...
func testHeaderChainImport(chain []*types.Header, lightchain *LightChain) error {
	for _, header := range chain {
		...
		rawdb.WriteTd(lightchain.chainDb, header.Hash(), header.Number.Uint64(), new(big.Int).Add(header.Difficulty, lightchain.GetTdByHash(header.ParentHash)), lightchain.hc.Config().IsFinalChain)
		rawdb.WriteHeader(lightchain.chainDb, header, lightchain.hc.Config().IsFinalChain)
