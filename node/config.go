...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gev is "gev". If no
...
// These resources are resolved differently for "gev" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gev 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'gev' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
