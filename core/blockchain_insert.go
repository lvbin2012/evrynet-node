...
func (st *insertStats) report(chain []*types.Block, index int, dirty common.StorageSize, isFinalChain bool) {
	...
	if index == len(chain)-1 || elapsed >= statsReportLimit {
		...
		context := []interface{}{
			...
			"number", end.Number(), "hash", end.Hash(), "isFinalChain", isFinalChain,
		}
