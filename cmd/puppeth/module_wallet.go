...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gev --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec gev --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --evrstats \'{{.Evrstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh
