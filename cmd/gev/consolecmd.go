...
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://github.com/Evrynetlabs/evrynet-node/wiki/JavaScript-Console.
This command allows to open a console on a running gev node.`,
	}
...
func localConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Failed to attach to the inproc gev: %v", err)
	}
...
func remoteConsole(ctx *cli.Context) error {
	// Attach to a remotely running gev instance and start the JavaScript console
	...
	if endpoint == "" {
		...
		endpoint = fmt.Sprintf("%s/gev.ipc", path)
	}
	client, err := dialRPC(endpoint)
	if err != nil {
		utils.Fatalf("Unable to attach to remote gev: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.GlobalString(utils.JSpathFlag.Name),
...
// for "gev attach" and "gev monitor" with no argument.
func dialRPC(endpoint string) (*rpc.Client, error) {
	if endpoint == "" {
		endpoint = node.DefaultIPCEndpoint(clientIdentifier)
	} else if strings.HasPrefix(endpoint, "rpc:") || strings.HasPrefix(endpoint, "ipc:") {
		// Backwards compatibility with gev < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
