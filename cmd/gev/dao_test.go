...
func testDAOForkBlockNewChain(t *testing.T, test int, genesis string, expectBlock *big.Int, expectVote bool) {
	...
	// Start a Gev instance with the requested flags set and immediately terminate
	if genesis != "" {
		...
		gev := runGev(t, append(args, []string{"--exec", "2+2", "console"}...)...)
		gev.WaitExit()
	}
	// Retrieve the DAO config flag from the database
	path := filepath.Join(datadir, "gev", "chaindata")
