...
type Downloader struct {
	...
	headerProcCh  chan *headerProcEvent // [eth/62] Channel to feed the header processor new tasks

	fHeaderCh      chan dataPack         // [eth/65] Channel receiving inbound block headers
	fBodyCh        chan dataPack         // [eth/65] Channel receiving inbound block bodies
	fReceiptCh     chan dataPack         // [eth/65] Channel receiving inbound receipts
...
func (d *Downloader) Synchronise(id string, head common.Hash, td *big.Int, mode SyncMode, isFinalChain bool) error {
	err := d.synchronise(id, head, td, mode, isFinalChain)
	switch err {
	case nil:
...
func (d *Downloader) synchronise(id string, hash common.Hash, td *big.Int, mode SyncMode, isFinalChain bool) error {
	queue := d.queue
	if isFinalChain {
		queue = d.fQueue
	}
	...
	return d.syncWithPeer(p, hash, td, isFinalChain)
}

// Synchronise tries to sync up our local block chain with a remote peer, both
// adding various sanity checks as well as wrapping it with various log entries.
func (d *Downloader) SynchroniseTwoChain(id string, head common.Hash, td *big.Int, fHead common.Hash, fTD *big.Int,
	mode SyncMode) error {
	err := d.synchroniseTwoChain(id, head, td, fHead, fTD, mode)
	switch err {
	case nil:
	case errBusy, errCanceled:
...
func (d *Downloader) synchroniseTwoChain(id string, head common.Hash, td *big.Int, fHead common.Hash, fTD *big.Int,
	...
	var dataPackChs []chan dataPack
	var headerProcChs []chan *headerProcEvent
	if head != (common.Hash{}) {
		d.queue.Reset()
		boolChs = append(boolChs, d.bodyWakeCh)
		boolChs = append(boolChs, d.receiptWakeCh)
		dataPackChs = append(dataPackChs, d.headerCh)
		dataPackChs = append(dataPackChs, d.bodyCh)
		dataPackChs = append(dataPackChs, d.receiptCh)
		headerProcChs = append(headerProcChs, d.headerProcCh)
	}

	if fHead != (common.Hash{}) {
		d.fQueue.Reset()
	...
	downloadFuc := func(p *peerConnection, head common.Hash, td *big.Int, isFinalChain bool) {
		if head == (common.Hash{}) {
			errc <- nil
			return
		}
		errc <- d.syncWithPeer(p, head, td, isFinalChain)
	}
	go downloadFuc(p, head, td, false)
	go downloadFuc(p, fHead, fTD, true)
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int, isFinalChain bool) (err error) {
	...
	defer func() {
		...
		if err != nil {
			...
			latest := lightchain.CurrentHeader()
	...
	latest, err := d.fetchHeight(p, isFinalChain)
	if err != nil {
		return err
	}
	height := latest.Number.Uint64()

	origin, err := d.findAncestor(p, latest, isFinalChain)
	if err != nil {
		return err
	}
	...
	if d.mode == FastSync {
		...
		if origin+1 < frozen {
			...
			for i := origin + 1; i < lightchain.CurrentHeader().Number.Uint64(); i++ {
				hashes = append(hashes, rawdb.ReadCanonicalHash(d.stateDB, i, blockchain.IsFinalChain()))
			}
			lightchain.Rollback(hashes)
		}
	}
	// Initiate the sync using a concurrent header and content retrieval algorithm
	queue.Prepare(origin+1, d.mode)
	if d.syncInitHook != nil {
		d.syncInitHook(origin, height)
	}
	fetchers := []func() error{
		func() error { return d.fetchHeaders(p, origin+1, pivot, isFinalChain) }, // Headers are always retrieved
		func() error { return d.fetchBodies(origin+1, isFinalChain) },            // Bodies are retrieved during normal and fast sync
	...
	if d.mode == FastSync {
		...
		fetchers = append(fetchers, func() error { return d.processFullSyncContent(isFinalChain) })
	}
	return d.spawnSync(fetchers, isFinalChain)
}

// spawnSync runs d.process and all given fetcher functions to completion in
// separate goroutines, returning the first error that appears.
func (d *Downloader) spawnSync(fetchers []func() error, isFinalChain bool) error {
	queue := d.queue
	if isFinalChain {
		queue = d.fQueue
	}
...
func (d *Downloader) fetchHeight(p *peerConnection, isFinalChain bool) (*types.Header, error) {
	...
	go p.peer.RequestHeadersByHash(head, 1, 0, false, isFinalChain)
	...
	for {
		...
		case packet := <-headerCh:
			if packet.IsFinalChain() != isFinalChain {
				panic(fmt.Sprintln("fetchHeight coming is FinalChain not equal ", packet.IsFinalChain(), isFinalChain))
				//d.headerCh <- packet
				//continue
			}
...
func (d *Downloader) findAncestor(p *peerConnection, remoteHeader *types.Header, isFinalChain bool) (uint64, error) {
	blockchain := d.blockchain
	lightchain := d.lightchain
	headerCh := d.headerCh
	bodyCh := d.bodyCh
	...
	switch d.mode {
	case FullSync:
		localHeight = blockchain.CurrentBlock().NumberU64()
	case FastSync:
		localHeight = blockchain.CurrentFastBlock().NumberU64()
	default:
		localHeight = lightchain.CurrentHeader().Number.Uint64()
	}
	...
	if d.mode == LightSync {
		...
		if *genesis == 0 {
			...
			for header != nil {
				*genesis = header.Number.Uint64()
				if floor >= int64(*genesis)-1 {
					break
				}
				header = lightchain.GetHeaderByHash(header.ParentHash)
			}
		}
		// We already know the "genesis" block number, cap floor to that
		if floor < int64(*genesis)-1 {
			floor = int64(*genesis) - 1
		}
	}
	...
	go p.peer.RequestHeadersByNumber(uint64(from), count, skip, false, isFinalChain)
	...
	for finished := false; !finished; {
		...
		case packet := <-headerCh:
			if packet.IsFinalChain() != isFinalChain {
				panic(fmt.Sprint("findAncestor coming is FinalChain not equal ", packet.IsFinalChain(), isFinalChain))
			}
			// Discard anything not from the origin peer
			if packet.PeerId() != p.id {
				log.Debug("Received headers from incorrect peer", "peer", packet.PeerId())
			...
			for i, header := range headers {
				...
				if number := header.Number.Int64(); number != expectNumber {
					p.log.Warn("Head headers broke chain ordering", "index", i, "requested", expectNumber, "received", number, "isFinalChain", isFinalChain)
			...
			for i := len(headers) - 1; i >= 0; i-- {
				...
				switch d.mode {
				case FullSync:
					known = blockchain.HasBlock(h, n)
				case FastSync:
					known = blockchain.HasFastBlock(h, n)
				default:
					known = lightchain.HasHeader(h, n)
				}
				if known {
					number, hash = n, h
					break
	...
	for start+1 < end {
		...
		go p.peer.RequestHeadersByNumber(check, 1, 0, false, isFinalChain)
		...
		for arrived := false; !arrived; {
			...
			case packer := <-headerCh:
				if packer.IsFinalChain() != isFinalChain {
					panic(fmt.Sprintln(" findAncestor coming is FinalChain not equal ", packer.IsFinalChain(), isFinalChain))
					//d.headerCh <- packer
					//continue
				}
				// Discard anything not from the origin peer
				...
				header := lightchain.GetHeaderByHash(h) // Independent of sync mode, header surely exists
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64, pivot uint64, isFinalChain bool) error {
	...
	getHeaders := func(from uint64) {
		...
		if skeleton {
			...
			go p.peer.RequestHeadersByNumber(from+uint64(MaxHeaderFetch)-1, MaxSkeletonSize, MaxHeaderFetch-1, false, isFinalChain)
		} else {
			p.log.Trace("Fetching full headers", "count", MaxHeaderFetch, "from", from)
			go p.peer.RequestHeadersByNumber(from, MaxHeaderFetch, 0, false, isFinalChain)
		}
	}
	...
	for {
		...
		case packet := <-headerCh:
			if packet.IsFinalChain() != isFinalChain {
				panic(fmt.Sprintln(" fetchHeaders coming is FinalChain not equal ", packet.IsFinalChain(), isFinalChain))
				//d.headerCh <- packet
				//continue
			}
			...
			if skeleton {
				filled, proced, err := d.fillHeaderSkeleton(from, headers, packet.IsFinalChain())
				if err != nil {
				...
				if n := len(headers); n > 0 {
					...
					head := uint64(0)
					if d.mode == LightSync {
						head = lightchain.CurrentHeader().Number.Uint64()
					} else {
						head = blockchain.CurrentFastBlock().NumberU64()
						if full := blockchain.CurrentBlock().NumberU64(); head < full {
							head = full
						}
					}
			...
			if len(headers) > 0 {
				...
				select {
				case headerProcCh <- &headerProcEvent{headers, packet.IsFinalChain()}:
				case <-d.cancelCh:
					return errCanceled
				}
...
func (d *Downloader) fillHeaderSkeleton(from uint64, skeleton []*types.Header, isFinalChain bool) ([]*types.Header, int, error) {
	...
	queue.ScheduleSkeleton(from, skeleton)

	var (
		deliver = func(packet dataPack) (int, error) {
			pack := packet.(*headerPack)
			return queue.DeliverHeaders(pack.peerID, pack.headers, pack.isFinalChain, headerProcCh)
		}
		expire   = func() map[string]int { return queue.ExpireHeaders(d.requestTTL()) }
		throttle = func() bool { return false }
		reserve  = func(p *peerConnection, count int, isFinalChain bool) (*fetchRequest, bool, error) {
			return queue.ReserveHeaders(p, count, isFinalChain), false, nil
		}
	...
	err := d.fetchParts(headerCh, deliver, queue.headerContCh, expire,
		...
		nil, fetch, queue.CancelHeaders, capacity, d.peers.HeaderIdlePeers, setIdle, "headers", isFinalChain)

	log.Debug("Skeleton fill terminated", "err", err)

	filled, proced := queue.RetrieveHeaders()
...
func (d *Downloader) fetchBodies(from uint64, isFinalChain bool) error {
	...
	var (
		deliver = func(packet dataPack) (int, error) {
			...
			return queue.DeliverBodies(pack.peerID, pack.transactions, pack.uncles)
		}
...
// and also periodically checking for timeouts.
func (d *Downloader) fetchReceipts(from uint64, isFinalChain bool) error {
	...
	var (
		deliver = func(packet dataPack) (int, error) {
			...
			return queue.DeliverReceipts(pack.peerID, pack.receipts)
		}
...
func (d *Downloader) fetchParts(deliveryCh chan dataPack, deliver func(dataPack) (int, error), wakeCh chan bool,
	expire func() map[string]int, pending func() int, inFlight func() bool, throttle func() bool, reserve func(*peerConnection, int, bool) (*fetchRequest, bool, error),
	fetchHook func([]*types.Header), fetch func(*peerConnection, *fetchRequest) error, cancel func(*fetchRequest), capacity func(*peerConnection) int,
	idle func() ([]*peerConnection, int), setIdle func(*peerConnection, int), kind string, isFinalChain bool) error {
	...
	for {
		...
		case packet := <-deliveryCh:
			if packet.IsFinalChain() != isFinalChain {
				return errors.New("flag isFinalChain not equal, kind is: " + kind)
			}
		...
		case <-update:
			...
			for _, peer := range idles {
				...
				// have them.
				request, progress, err := reserve(peer, capacity(peer), isFinalChain)
				if err != nil {
					return err
				}
				if progress {
...
func (d *Downloader) processHeaders(origin uint64, pivot uint64, td *big.Int, isFinalChain bool) error {
	queue := d.queue
	lightchain := d.lightchain
	blockchain := d.blockchain
	headerProcCh := d.headerProcCh
	...
	defer func() {
		if len(rollback) > 0 {
			...
			lastHeader, lastFastBlock, lastBlock := lightchain.CurrentHeader().Number, common.Big0, common.Big0
			if d.mode != LightSync {
				lastFastBlock = blockchain.CurrentFastBlock().Number()
				lastBlock = blockchain.CurrentBlock().Number()
			}
			lightchain.Rollback(hashes)
			curFastBlock, curBlock := common.Big0, common.Big0
			if d.mode != LightSync {
				curFastBlock = blockchain.CurrentFastBlock().Number()
				curBlock = blockchain.CurrentBlock().Number()
			}
			log.Warn("Rolled back headers", "count", len(hashes),
				"header", fmt.Sprintf("%d->%d", lastHeader, lightchain.CurrentHeader().Number),
		...
		case procEvent := <-headerProcCh:
			...
			if procEvent == nil || len(procEvent.headers) == 0 {
				...
				if d.mode != LightSync {
					...
					if !gotHeaders && td.Cmp(blockchain.GetTd(head.Hash(), head.NumberU64())) > 0 {
				...
				if d.mode == FastSync || d.mode == LightSync {
					head := lightchain.CurrentHeader()
					if td.Cmp(lightchain.GetTd(head.Hash(), head.Number.Uint64())) > 0 {
						return errStallingPeer
					}
				}
			...
			if procEvent.isFinalChain != isFinalChain {
				panic(fmt.Sprintln("isFinalChain is not equal", procEvent.isFinalChain, isFinalChain))
				//d.headerProcCh <- procEvent
				//continue
			}
			// Otherwise split the chunk of headers into batches and process them
			gotHeaders = true
			for len(procEvent.headers) > 0 {
				...
				if limit > len(procEvent.headers) {
					limit = len(procEvent.headers)
				}
				chunk := procEvent.headers[:limit]
				// In case of header only syncing, validate the chunk immediately
				if d.mode == FastSync || d.mode == LightSync {
					// Collect the yet unknown headers to mark them as uncertain
					unknown := make([]*types.Header, 0, len(chunk))
					for _, header := range chunk {
						if !lightchain.HasHeader(header.Hash(), header.Number.Uint64()) {
					...
					if n, err := lightchain.InsertHeaderChain(chunk, frequency); err != nil {
				...
				if d.mode == FullSync || d.mode == FastSync {
					...
					for queue.PendingBlocks() >= maxQueuedHeaders || queue.PendingReceipts() >= maxQueuedHeaders {
						select {
						case <-d.cancelCh:
							return errCanceled
					...
					inserts := queue.Schedule(chunk, origin)
					if len(inserts) != len(chunk) {
						log.Debug("Stale headers")
						return errBadPeer
					}
				}
				procEvent.headers = procEvent.headers[limit:]
				origin += uint64(limit)
			}
...
func (d *Downloader) processFullSyncContent(isFinalChain bool) error {
	queue := d.queue
	if isFinalChain {
		queue = d.fQueue
	}
	for {
		results := queue.Results(true)
		if len(results) == 0 {
			return nil
		}
		...
		if err := d.importBlockResults(results, isFinalChain); err != nil {
			return err
		}
	}
}

func (d *Downloader) importBlockResults(results []*fetchResult, isFinalChain bool) error {
	...
	if index, err := blockchain.InsertChain(blocks); err != nil {
...
func (d *Downloader) processFastSyncContent(latest *types.Header, isFinalChain bool) error {
	...
	stateSync := d.syncState(latest.Root, isFinalChain)
	defer stateSync.Cancel()
	go func() {
		if err := stateSync.Wait(); err != nil && err != errCancelStateFetch && err != errCanceled {
			queue.Close() // wake up Results
		}
	}()
	...
	for {
		...
		results := queue.Results(oldPivot == nil) // Block if we're not monitoring pivot staleness
		if len(results) == 0 {
		...
		if P != nil {
			...
			if oldPivot != P {
				...
				stateSync = d.syncState(P.Header.Root, isFinalChain)
				defer stateSync.Cancel()
				go func() {
					if err := stateSync.Wait(); err != nil && err != errCancelStateFetch && err != errCanceled {
						queue.Close() // wake up Results
		...
		if err := d.importBlockResults(afterP, isFinalChain); err != nil {
			return err
		}
	}
}
...
func (d *Downloader) DeliverHeaders(id string, isFinalChain bool, headers []*types.Header) (err error) {
	if isFinalChain {
		return d.deliver(id, d.fHeaderCh, &headerPack{id, isFinalChain, headers}, headerInMeter, headerDropMeter)
	}
	return d.deliver(id, d.headerCh, &headerPack{id, isFinalChain, headers}, headerInMeter, headerDropMeter)
}

// DeliverBodies injects a new batch of block bodies received from a remote node.
func (d *Downloader) DeliverBodies(id string, isFinalChain bool, transactions [][]*types.Transaction, uncles [][]*types.Header) (err error) {
	if isFinalChain {
		return d.deliver(id, d.fBodyCh, &bodyPack{id, isFinalChain, transactions, uncles}, bodyInMeter, bodyDropMeter)
	}
	return d.deliver(id, d.bodyCh, &bodyPack{id, isFinalChain, transactions, uncles}, bodyInMeter, bodyDropMeter)
}

// DeliverReceipts injects a new batch of receipts received from a remote node.
func (d *Downloader) DeliverReceipts(id string, isFinalChain bool, receipts [][]*types.Receipt) (err error) {
	if isFinalChain {
